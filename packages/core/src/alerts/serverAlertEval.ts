...
export function evaluateServerAlert(alert: Alert, state: MirrorState): AlertEvalResult {
  ...
  if (
    alert.snoozedUntilISO &&
    new Date(alert.snoozedUntilISO).getTime() > new Date(state.nowISO).getTime()
  ) {
  ...
  switch (alert.type) {
    case 'PRICE': {
      ...
      if (!alert.thresholdBase)
        return { triggered: false, context: { reason: 'missing thresholdBase' } };
      ...
      const ok =
        dir === 'ABOVE'
          ? current.greaterThanOrEqualTo(threshold)
          : current.lessThanOrEqualTo(threshold);
      return ok
        ? {
            triggered: true,
            context: {
    ...
    case 'DRAWDOWN': {
      if (!alert.thresholdPct)
        return { triggered: false, context: { reason: 'missing thresholdPct' } };
    ...
    case 'PCT_CHANGE': {
      ...
      if (!currentPrice || !history || history.length === 0)
        return { triggered: false, context: { reason: 'missing history' } };
    ...
    case 'DRIFT': {
      ...
      if (!drifts || Object.keys(drifts).length === 0)
        return { triggered: false, context: { reason: 'missing drift' } };
      const hit = Object.entries(drifts).find(([, v]) =>
        dec(v).abs().greaterThanOrEqualTo(threshold),
      );
      return hit
        ? {
            triggered: true,
    ...
    case 'TAKE_PROFIT': {
      if (!alert.assetId || !alert.thresholdBase)
        return { triggered: false, context: { reason: 'missing fields' } };
      const price = state.assetPrices[alert.assetId];
      if (!price) return { triggered: false, context: { reason: 'missing price' } };
