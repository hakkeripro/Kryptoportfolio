  ...
  const topPositions = useMemo(() => {
    ...
    return [...latest.positions]
      ...
      .map((p) => ({
        ...
        iconUrl: byId.get(p.assetId)?.iconUrl,
      }));
  }, [dbState.data.latest, dbState.data.assets]);
          ...
          {allocation.length ? (
            <div className="h-64 mt-3">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface-overlay)', border: '1px solid var(--color-border)', borderRadius: '0.5rem', color: 'var(--color-content-primary)' }} />
                  <Pie data={allocation} dataKey="value" nameKey="name" innerRadius={50} outerRadius={90}>
                    {allocation.map((x) => (<Cell key={x.assetId} fill={colorForKey(x.assetId)} />))}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="h-64 mt-3">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={valueSeries}>
                  ...
                  <Line type="monotone" dataKey="value" dot={false} stroke="var(--color-brand)" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          ) : (
              ...
              {topPositions.map((p) => (
                <li key={p.assetId} className="py-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <TokenIcon symbol={p.symbol} iconUrl={p.iconUrl} size="sm" />
                    <span className="text-body text-content-primary font-medium">{p.symbol}</span>
                  </div>
                  <span className="text-body font-mono text-content-primary">{fmtMoney(p.valueBase, baseCurrency)}</span>
                </li>
              ))}
            </ul>
          ) : (
