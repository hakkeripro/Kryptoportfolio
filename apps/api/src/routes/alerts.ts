...
export function registerAlertRoutes(app: FastifyInstance) {
  app.post('/v1/alerts/server/enable', { preHandler: requireAuth }, async (req) => {
    ...
    const body = EnableServerAlertsSchema.parse(req.body);
    const now = new Date().toISOString();

    const alerts = body.alerts ?? [];
    const mode = body.mode ?? (alerts.length ? 'replace' : 'enable_only');

    const existingAlerts = app.db.query<any>(
      'SELECT id, lastTriggeredAtISO FROM server_alerts WHERE userId=?',
      [userId],
    );
    const lastById = new Map(
      existingAlerts.map((r: any) => [String(r.id), r.lastTriggeredAtISO ?? null] as const),
    );

    if (mode === 'enable_only') {
      app.db.exec('UPDATE server_alerts SET isEnabled=1, updatedAtISO=? WHERE userId=?', [
        now,
        userId,
      ]);
      ...
      for (const a of alerts) {
        const lastISO =
          (a as any).lastTriggeredAtISO ?? lastById.get(String((a as any).id)) ?? null;
      ...
      for (const a of alerts) {
        ...
        const alertJson = JSON.stringify({ ...a, lastTriggeredAtISO: lastISO });
        const exists = app.db.getOne<any>('SELECT id FROM server_alerts WHERE id=? AND userId=?', [
          id,
          userId,
        ]);
        if (exists) {
          app.db.exec(
            'UPDATE server_alerts SET alertJson=?, isEnabled=?, updatedAtISO=?, lastTriggeredAtISO=? WHERE id=? AND userId=?',
            [alertJson, a.isEnabled ? 1 : 0, a.updatedAtISO ?? now, lastISO, id, userId],
          );
        } else {
          app.db.exec(
            'INSERT INTO server_alerts(id,userId,alertJson,isEnabled,createdAtISO,updatedAtISO,lastTriggeredAtISO) VALUES (?,?,?,?,?,?,?)',
    ...
    // upsert mirror state
    const existingState = app.db.getOne<{ userId: string }>(
      'SELECT userId FROM alert_mirror_state WHERE userId=?',
      [userId],
    );
    if (existingState) {
    ...
    return {
      ok: true,
      evaluated: (evalRes as any).evaluated ?? 0,
      triggered: (evalRes as any).triggered ?? 0,
    };
  });
  ...
  app.post('/v1/alerts/server/disable', { preHandler: requireAuth }, async (req) => {
    const userId = getUserId(req);
    const now = new Date().toISOString();
    app.db.exec('UPDATE server_alerts SET isEnabled=0, updatedAtISO=? WHERE userId=?', [
      now,
      userId,
    ]);
    return { ok: true };
  ...
  app.post('/v1/alerts/server/state', { preHandler: requireAuth }, async (req) => {
    const userId = getUserId(req);
    const body = MirrorStateBodySchema.parse(req.body);
    const now = new Date().toISOString();

    const existing = app.db.getOne<{ userId: string }>(
      'SELECT userId FROM alert_mirror_state WHERE userId=?',
      [userId],
    );
    if (existing) {
  ...
  app.get('/v1/alerts/server/status', { preHandler: requireAuth }, async (req) => {
    const userId = getUserId(req);
    const enabled = app.db.query<any>(
      'SELECT id FROM server_alerts WHERE userId=? AND isEnabled=1',
      [userId],
    ).length;
    const total = app.db.query<any>('SELECT id FROM server_alerts WHERE userId=?', [userId]).length;
    const stateRow = app.db.getOne<any>(
      'SELECT updatedAtISO FROM alert_mirror_state WHERE userId=?',
      [userId],
    );
    return {
      ok: true,
      ...
      totalCount: total,
      mirrorUpdatedAtISO: stateRow?.updatedAtISO ?? null,
    };
  });

  app.get('/v1/alerts/server/log', { preHandler: requireAuth }, async (req) => {
    const userId = getUserId(req);
    const q = TriggerLogQuerySchema.parse(req.query);
    ...
    return { logs: rows.map(mapTriggerLogRow) };
  });
}
