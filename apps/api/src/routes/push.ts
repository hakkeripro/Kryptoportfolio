...
export function registerPushRoutes(app: FastifyInstance) {
  ...
  app.post('/v1/push/web/subscribe', { preHandler: requireAuth }, async (req, reply) => {
    ...
    const body = WebPushSubscribeSchema.parse(req.body);
    ...
    if (existing) {
      app.db.exec(
        'UPDATE web_push_subscriptions SET subscriptionJson=?, updatedAtISO=? WHERE id=?',
        [JSON.stringify(sub), now, existing.id],
      );
    } else {
      app.db.exec(
  ...
  app.post('/v1/push/web/unsubscribe', { preHandler: requireAuth }, async (req) => {
    const userId = getUserId(req);
    const body = WebPushUnsubscribeSchema.parse(req.body);
    app.db.exec('DELETE FROM web_push_subscriptions WHERE userId=? AND endpoint=?', [
      userId,
      body.endpoint,
    ]);
    return { ok: true };
  ...
  app.post('/v1/push/expo/register', { preHandler: requireAuth }, async (req) => {
    const userId = getUserId(req);
    const body = ExpoPushTokenSchema.parse(req.body);
    const now = new Date().toISOString();
    const existing = app.db.getOne<{ id: string }>(
      'SELECT id FROM expo_push_tokens WHERE token=?',
      [body.token],
    );
